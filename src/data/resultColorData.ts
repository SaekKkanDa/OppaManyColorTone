...
const springBright: ColorResult = {
  ...
  description:
    '웜톤이지만 쿨톤의 영향을 많이 받은 봄 브라이트는 밝은 에너지와 대비감이 중요한 타입이에요. 너무 인위적이지 않은 쿨톤의 색과 너무 더운색을 제외한 따뜻한 색까지 활용할 수 있는 스펙트럼이 넓어요. 형형색색의 채도 높은 색들을 잘 소화해 캐주얼한 스타일링도 잘 어울리지만, 너무 강한 형광기는 피해주세요. 봄의 러블리한 꽃 프린팅 의상도 잘 소화하지만, 너무 대비감이 없는 톤온톤 스타일링은 노숙한 스타일링은 피해주세요.',
...
const springWarm: ColorResult = {
  ...
  description:
    '봄웜은 봄 컬러 중 색의 온도가 가장 중요한 타입이에요. 한국인 중에 제일 희귀한 타입인 봄웜은 오렌지를 누구보다 잘 소화할 수 있는 타입이에요. 생기발랄한 스타일과 장난스럽고 러블리한 스타일의 봄웜은 회기가 심하고 대비감이 없는 스타일링은 존재감이 약해 보일수 있으니 피해주세요.',
...
const springLight: ColorResult = {
  ...
  description:
    '봄라이트는 생기, 활력, 풋풋한 봄의 색을 가장 잘 쓰는 타입이에요. 아이보리, 코랄, 노랑, 연두색을 다른 어떤 타입보다 잘 소화해요. 연한 웜톤 계열의 색은 다 베스트이고, 연한 쿨톤의 색상도 세컨드 베스트로 활용할 수 있어요. 너무 창백한 쿨한 색상이나, 회기가 많이 들어간 색은 힘이 빠져보여요. 딥한 저명도 색상은 평소 이미지보다 성숙해보이게 만들어요.',
...
const summerLight: ColorResult = {
  ...
  description:
    '여름라이트는 여름 계절중 가장 맑고 화사해요. 여름의 베스트 컬러인 화이트를 잘 활용해주면 좋아요. 대비감을 필요로하지만 블랙앤화이트 보다는 흰티셔츠에 청바지의 조화가 베스트에요! 여름라이트는 힙하고, 펑키하고, 써지컬, 스트릿패션은 다소 소화하기 어려워요.',
...
const summerCool: ColorResult = {
  ...
  description:
    '여름쿨은 여름 계절 중 색의 온도가 가장 중요한 타입이에요. 시원한 파란색을 다른 타입보다 잘 소화하며 전형적인 쿨톤 색은 다 잘어울려요. 블랙도 잘 소화하지만 블루블랙은 너무 차가워 보일수 있어요. 대비감이 중요하기 때문에 패션이나 악세서리에 대비감을 넣은 스타일링이 잘 어울려요.',
...
const summerMute: ColorResult = {
  ...
  description:
    '여름뮤트는 쿨한 여름이지만 부드러운 이미지가 있는 타입이에요. 여름 타입의 색 중에서 탁한 느낌이 드는 저채도의 색은 다 잘 어울리며, 심하게 광이 들어간 색은 조화가 깨지고 어색해 보일수 있어요. 또한 쨍한 블랙은 무겁기 때문에 블랙브라운이나 차콜을 활용해 주는게 좋아요. 탁한 느낌과 대비되는 형광기는 없애주고, 너무 강한 대비감도 피해주세요.',
...
const autumnMute: ColorResult = {
  ...
  description:
    '가을뮤트는 가을 계절에서 부드럽고 뽀얀 느낌이 드는 타입이에요. 소프트한 아우라를 크게 가지고 있기 때문에 비비드 같은 고채도 색상은 인위적으로 보일 수 있어요. 또한 명도대비를 많이 벌린 블랙앤화이트 보다는 베이지, 오트밀, 다크브라운 같은 부드러운 색이 잘 어울려요. 톤온톤 패션을 잘 소화하고, 비슷한계열의 색으로는 대비감 넣어주는것도 좋아요. 큼직한 무늬와 악세서리를 잘 소화하기 때문에 악세서리를 활용한 패션이 잘 어울려요.',
...
const autumnWarm: ColorResult = {
  ...
  description:
    '가을웜은 가을 계절 중 색의 따뜻한 온도가 가장 중요한 타입이에요. 레드, 카키 그린, 베이지, 브라운 등 웜톤 색이 특히나 잘 어울리며 인공적인 컬러는 소화력이 낮아요. 너무 심플한 스타일은 허전해 보일수 있고, 음영 메이크업이 굉장히 잘 어울려요. 잔무늬가 많고 회기가 심하면 힘이 빠져보여요. 대비감이 들어간 스타일링, 체크무늬, 니트가 굉장히 잘 어울려요.',
...
const autumnDeep: ColorResult = {
  ...
  description:
    '가을딥은 진하고 딥한 웜색을 가장 잘 받는 타입이에요. 가을딥의 매력을 돋보이게 하려면 대비감이 강한 패션이 중요해요. 블랙 소화력이 높으나 인위적인 블랙보다는 부드러운 블랙이 베스트에요. 빈티지한 소품과 체크무늬, 니트, 모직, 털 소화력이 높아요. 악세서리는 너무 작은것보다는 크고 화려한게 잘 어울려요. 비비드한 컬러는 자칫 촌스러워 보일수 있고, 음영을 활영한 메이크업과 스모키 메이크업이 굉장히 잘 어울려요.',
...
const winterDeep: ColorResult = {
  ...
  description:
    '겨울딥은 심플함과 깨끗함이 베스트인 타입이에요. 깨끗하고 선명한 에너지를 갖고 있는 이 타입은 무던한 블랙앤화이트 코디가 무척 잘 어울려요. 화이트와 거의 화이트에 가까운 아이시(icy)컬러는 다 잘 어울리며, 지저분해 보일수 있는 저대비, 잔무늬는 소화하기 어려워요. 화려한 악세서리는 과해 보일수 있고 악세서리를 착용하지 않아도 밋밋해보이지 않아요.',
...
const winterCool: ColorResult = {
  ...
  description:
    '겨울쿨은 겨울 계절 중 쿨한 색의 온도가 가장 중요한 타입이에요. 차가운 느낌의 쿨톤 색상이 잘 어울리는 이 타입은 겨울의 광택감을 잘 소화해요. 재질이 깔끔한게 중요하기 때문에 잔무늬는 소화하기 어렵고, 악세서리도 많이 필요하지 않아요. 스포티, 캐주얼한 스타일링을 잘 소화하고 힘빠지는 옷 재질은 주의해주세요.',
...
const winterBright: ColorResult = {
  ...
  description:
    '겨울브라이트는 클리어한 속성이 중요한 타입이에요. 전체적으로 선명하고 초롱초롱한 이미지의 이 타입은 대비감과 광택감이 잘 어울려요. 블랙 컬러 소화력이 좋으며 라이더 재킷과 광이 많은 벨트, 악세서리를 활용해 주면 좋아요. 사용할 수 있는 레드 팔레트가 오렌지레드부터 쿨레드까지 스펙트럼이 넓어요. 대비감이 없거나 탁한색, 너무 촘촘한 무늬는 주의해주세요.',
